  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-2 gap-6">
      {sampleImages.map((image, index) => (
        <div 
          key={index}
          className="relative group overflow-hidden rounded-lg shadow-md hover:shadow-xl transition-all cursor-pointer border border-gray-200 hover:border-indigo-300"
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-70 group-hover:opacity-60 transition-opacity" />
          <div className="absolute bottom-0 left-0 right-0 p-3 transform transition-transform group-hover:translate-y-0 duration-300">
            <span className="text-white text-base font-medium block truncate mb-1">{image.description}</span>
            <div className="flex items-center">
              {image.isHandwriting ? (
                <span className="flex items-center text-sm text-amber-300 bg-black/30 px-2 py-1 rounded-full">
                  <Edit3 size={14} className="mr-1" />
                  Handwritten
                </span>
              ) : (
                <span className="flex items-center text-sm text-blue-300 bg-black/30 px-2 py-1 rounded-full">
