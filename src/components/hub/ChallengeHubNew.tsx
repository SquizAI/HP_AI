...
const ChallengeHubNew: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
              ...
              <div key={challenge.id} className="relative overflow-hidden w-full max-w-sm md:max-w-none">
                <Link to={challenge.path} className="block h-full">
                  <div 
                    ...
                    isCompleted(challenge.id) 
                      ? 'bg-gradient-to-r from-pink-600/90 to-purple-700/90 text-white border border-purple-400' 
                  ...
                  >
                    {/* Card Header with Modality Icon */}
                    <div className={`flex items-center justify-between p-3 transition-all duration-300 ${isCompleted(challenge.id) ? 'bg-gradient-to-r from-pink-800/40 to-purple-900/40' : 'bg-[#0096D6]/15'}`}>
                      {/* Modality Icon */}
                      <div className="flex items-center">
                        {modalityTypes[challenge.modality as keyof typeof modalityTypes]?.icon ? (
                  ...
                  <div className="absolute bottom-3 right-3 flex">
                    {isCompleted(challenge.id) ? (
                      ...
                      <button
                        ...
                        className="p-2 bg-gradient-to-r from-pink-600 to-purple-600 rounded-full shadow-md hover:from-pink-700 hover:to-purple-700 hover:scale-105 transition-all duration-200 border border-purple-400"
                        title="Mark as completed"
                        aria-label="Mark challenge as completed"
                      >
                        <CheckCircle className="h-5 w-5 text-white" />
      ...
      {/* Confetti effect when completing HP challenges */}
      {showConfetti && (
        <ReactConfetti
          ...
          recycle={false}
          numberOfPieces={500}
          colors={['#e91e63', '#9c27b0', '#3f51b5', '#673ab7', '#0096D6']}
        />
      )}
    </div>
  );
